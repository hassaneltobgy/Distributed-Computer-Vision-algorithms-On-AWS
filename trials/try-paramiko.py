...
def send_request(filename, operation, host, port, request_id, width=None, height=None):
    ...
    if message == b"IMAGE RECEIVED":
        ...
        # Send selected operation to server
        client_socket.send(f"{operation}".encode())

        # Send additional parameters if scaling is selected
      ...
      <body>
        <div class="container">
          ...
          <form id="image-form" onsubmit="processImage(event)">
            ...
            <select id="operation" name="operation" onchange="showHideFields()">
        ...
        <script>
          function showHideFields() {
            ...
            var scalingFields = document.getElementById("scalingFields");
            if (operation == "scaling") {
...
def process_image():
    ...
    thread = threading.Thread(target=send_request, args=(filename, operation, host, port, request_id, width, height))
    thread.start()
...
def download_processed_image():
    ...
    processed_images = [f for f in os.listdir('.') if f.startswith("processed_image_") and f.endswith(".jpg")]
    if processed_images:
        latest_image = max(processed_images, key=os.path.getctime)
        return send_file(latest_image, as_attachment=True)
    else:
